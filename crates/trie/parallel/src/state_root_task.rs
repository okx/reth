...
impl StateRootHandle {
    ...
    pub fn state_hook_persistent(&self) -> impl alloy_evm::block::OnStateHook {
        // Use the raw sender (not `StateHookSender`) so dropping the hook does not emit
        // `StateRootMessage::FinishedStateUpdates`; the orchestrator finalizes the pipeline by
        // using the standard `state_hook` for the terminal execution.
