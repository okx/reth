    ...
    Attrs: OpAttributes<Transaction = TxTy<Evm::Primitives>>,
{
        ...
        <<Builder::Executor as BlockExecutor>::Evm as AlloyEvm>::DB: Database,
    {
        ...
        while let Some(tx) = best_txs.next(()) {
            ...
            //
            // Gasless (zero-priced) txs have no tip when the block base fee is non-zero
            // (`effective_tip_per_gas` returns `None`) and contribute no miner fee, so an `expect`
            // here would panic. Treat a missing tip as 0.
