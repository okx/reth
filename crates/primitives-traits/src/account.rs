...
impl reth_codecs::Compact for Bytecode {
        ...
        B: bytes::BufMut + AsMut<[u8]>,
    {
        ...
        let len = if self.0.is_eip7702() {
            ...
            // Legacy analyzed (REMOVED_BYTECODE_ID has been removed).
            buf.put_u8(LEGACY_ANALYZED_BYTECODE_ID);
            let original_len = self.0.original_byte_slice().len();
            buf.put_u64(original_len as u64);
            let jump_table = self.0.legacy_jump_table().expect("legacy bytecode has jump table");
...
#[cfg(any(test, feature = "reth-codec"))]
mod compress_impls {
    use super::{Account, Bytecode};
    use reth_codecs::{Compress, Decompress};
    reth_codecs::impl_compression_for_compact!(Account, Bytecode);
}
