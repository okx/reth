...
impl LazyOverlay {
    ...
    fn merge_blocks(blocks: &[DeferredTrieData]) -> TrieInputSorted {
        ...
        if blocks.len() < MERGE_BATCH_THRESHOLD {
            ...
            let merged_state: Arc<HashedPostStateSorted> =
                HashedPostStateSorted::merge_batch(trie_data.iter().map(|d| d.hashed_state.clone()));
            let merged_nodes: Arc<TrieUpdatesSorted> =
                TrieUpdatesSorted::merge_batch(trie_data.iter().map(|d| d.trie_updates.clone()));

            TrieInputSorted { state: merged_state, nodes: merged_nodes, prefix_sets: Default::default() }
        }
    }
}
