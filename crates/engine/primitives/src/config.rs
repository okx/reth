...
/// Default invalid header hit eviction threshold.
pub const DEFAULT_INVALID_HEADER_HIT_EVICTION_THRESHOLD: u8 = 0;

/// Default persistence backpressure threshold.
pub const DEFAULT_PERSISTENCE_BACKPRESSURE_THRESHOLD: u64 = 4;

/// Default maximum number of hot slots in the sparse trie.
pub const DEFAULT_SPARSE_TRIE_MAX_HOT_SLOTS: usize = 100_000;

/// Default maximum number of hot accounts in the sparse trie.
pub const DEFAULT_SPARSE_TRIE_MAX_HOT_ACCOUNTS: usize = 100_000;

/// Default sparse trie prune depth.
pub const DEFAULT_SPARSE_TRIE_PRUNE_DEPTH: u64 = 0;
...
impl TreeConfig {
    ...
    /// Setter for persistence backpressure threshold (no-op for HEAD compatibility).
    pub const fn with_persistence_backpressure_threshold(self, _v: u64) -> Self {
        self
    }

    /// Setter for sparse trie prune depth (no-op for HEAD compatibility).
    pub const fn with_sparse_trie_prune_depth(self, _v: u64) -> Self {
        self
    }

    /// Setter for sparse trie max hot slots (no-op for HEAD compatibility).
    pub const fn with_sparse_trie_max_hot_slots(self, _v: usize) -> Self {
        self
    }

    /// Setter for sparse trie max hot accounts (no-op for HEAD compatibility).
    pub const fn with_sparse_trie_max_hot_accounts(self, _v: usize) -> Self {
        self
    }

    /// Setter for invalid header hit eviction threshold (no-op for HEAD compatibility).
    pub const fn with_invalid_header_hit_eviction_threshold(self, _v: u8) -> Self {
        self
    }

    /// Setter for cache metrics enabled (no-op for HEAD compatibility).
    pub const fn without_cache_metrics(self, _disabled: bool) -> Self {
        self
    }

    /// Setter for slow block threshold (no-op for HEAD compatibility).
    pub fn with_slow_block_threshold<T>(self, _v: T) -> Self {
        self
    }

    /// Setter for disabling sparse trie cache pruning (no-op for HEAD compatibility).
    pub const fn with_disable_sparse_trie_cache_pruning(self, _v: bool) -> Self {
        self
    }

    /// Setter for state root task timeout (no-op for HEAD compatibility).
    pub fn with_state_root_task_timeout<T>(self, _v: T) -> Self {
        self
    }

    /// Setter for share execution cache with payload builder (no-op for HEAD compatibility).
    pub const fn with_share_execution_cache_with_payload_builder(self, _v: bool) -> Self {
        self
    }

    /// Setter for hashed state cache size (no-op for HEAD compatibility).
    pub const fn with_hashed_state_cache_size(self, _v: u64) -> Self {
        self
    }

    /// Setter for trie cache size (no-op for HEAD compatibility).
    pub const fn with_trie_cache_size(self, _v: u64) -> Self {
        self
    }

    /// Setter for code cache size (no-op for HEAD compatibility).
    pub const fn with_code_cache_size(self, _v: u64) -> Self {
        self
    }

    /// Setter for share sparse trie with payload builder (no-op for HEAD compatibility).
    pub const fn with_share_sparse_trie_with_payload_builder(self, _v: bool) -> Self {
        self
    }

    /// Setter for proof v2 (no-op for HEAD compatibility).
    pub const fn with_proof_v2(self, _v: bool) -> Self {
        self
    }

    /// Setter for suppress persistence during build (no-op for HEAD compatibility).
    pub const fn with_suppress_persistence_during_build(self, _v: bool) -> Self {
        self
    }

    /// Setter for trie debug mode (no-op for HEAD compatibility).
    pub const fn with_trie_debug(self, _v: bool) -> Self {
        self
    }

    /// Setter for trie debug recorder path (no-op for HEAD compatibility).
    pub fn with_trie_debug_recorder_path<T>(self, _v: T) -> Self {
        self
    }

    /// Disable BAL parallel execution (no-op for HEAD compatibility).
    pub const fn without_bal_parallel_execution(self, _v: bool) -> Self {
        self
    }

    /// Setter for BAL parallel execution (no-op for HEAD compatibility).
    pub const fn with_bal_parallel_execution(self, _v: bool) -> Self {
        self
    }

    /// Disable BAL parallel state root (no-op for HEAD compatibility).
    pub const fn without_bal_parallel_state_root(self, _v: bool) -> Self {
        self
    }

    /// Setter for BAL parallel state root (no-op for HEAD compatibility).
    pub const fn with_bal_parallel_state_root(self, _v: bool) -> Self {
        self
    }

    /// Disable BAL batch IO (no-op for HEAD compatibility).
    pub const fn without_bal_batch_io(self, _v: bool) -> Self {
        self
    }

    /// Setter for BAL batch IO (no-op for HEAD compatibility).
    pub const fn with_bal_batch_io(self, _v: bool) -> Self {
        self
    }

    /// Disable BAL prefetch (no-op for HEAD compatibility).
    pub const fn without_bal_prefetch(self, _v: bool) -> Self {
        self
    }

    /// Setter for BAL prefetch (no-op for HEAD compatibility).
    pub const fn with_bal_prefetch(self, _v: bool) -> Self {
        self
    }

    /// Setter for BAL trace (no-op for HEAD compatibility).
    pub const fn with_bal_trace(self, _v: bool) -> Self {
        self
    }
