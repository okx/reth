    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
            ...
            + 'static,
    {
        ...
        // Reuse a caller-provided state root pipeline when present (e.g. flashblocks spawn
        // incrementally and preserve a single pipeline across spawns); otherwise spawn a fresh one.
...
mod tests {
    ...
    fn test_state_root() {
        ...
        let mut handle = payload_processor.spawn(
            ...
            &TreeConfig::default(),
            false, // not parallel BAL execution
            None,  // no preserved state root handle for test
        );
