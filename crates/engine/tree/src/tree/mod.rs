    ...
    V: EngineValidator<T>,
{
        ...
        Err: From<InsertBlockError<N::Block>>,
    {
        ...
        match self.sealed_header_by_hash(block_num_hash.hash) {
            ...
            Ok(Some(_)) => {
                info!(target: "engine::tree", block=?block_num_hash, "XXX Block hash already exists in tree");
        ...
        info!(target: "engine::tree", block=?block_num_hash, "XXX Block hash does not exists in tree");
