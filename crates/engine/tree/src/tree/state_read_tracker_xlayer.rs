//! X Layer: State read time tracking for block execution timing metrics.
//!
//! This module provides a lightweight wrapper for tracking state read operations
//! (account, storage, and bytecode reads) with minimal overhead.
...
/// Trait for types that can track IO read time
pub(crate) trait IoReadTracker {
    /// Returns the total IO read time
    fn total_io_read_time(&self) -> Duration;
}

/// A lightweight wrapper of a state provider that tracks state read time (account + storage + bytecode reads).
/// This is used in both Build and Insert phases to track state read time with minimal overhead.
pub(crate) struct StateReadTracker<S> {
    /// The state provider
    state_provider: S,
    /// The total time spent on state read operations (account + storage + code fetches)
    total_io_read: AtomicDuration,
}

impl<S> StateReadTracker<S>
where
    S: StateProvider,
{
    /// Creates a new [`StateReadTracker`] from a state provider
    pub(crate) fn from_state_provider(state_provider: S) -> Self {
        Self {
            state_provider,
            total_io_read: AtomicDuration::zero(),
        }
    }
}

impl<S> IoReadTracker for StateReadTracker<S> {
    /// Returns the total IO read time (account + storage + code fetches)
    fn total_io_read_time(&self) -> Duration {
        self.total_io_read.duration()
    }
}

impl<S> StateReadTracker<S> {
    /// Records the latency for a storage fetch
    fn record_storage_fetch(&self, latency: Duration) {
        self.total_io_read.add_duration(latency);
    }

    /// Records the latency for a code fetch
    fn record_code_fetch(&self, latency: Duration) {
        self.total_io_read.add_duration(latency);
    }

    /// Records the latency for an account fetch
    fn record_account_fetch(&self, latency: Duration) {
        self.total_io_read.add_duration(latency);
    }
}

impl<S: AccountReader> AccountReader for StateReadTracker<S> {
...
impl<S: StateProvider> StateProvider for StateReadTracker<S> {
    fn storage(
        &self,
        account: Address,
...
impl<S: BytecodeReader> BytecodeReader for StateReadTracker<S> {
...
impl<S: StateRootProvider> StateRootProvider for StateReadTracker<S> {
...
impl<S: StateProofProvider> StateProofProvider for StateReadTracker<S> {
    fn proof(
        &self,
        input: TrieInput,
        address: Address,
...
impl<S: StorageRootProvider> StorageRootProvider for StateReadTracker<S> {
    fn storage_root(
        &self,
        address: Address,
...
impl<S: BlockHashReader> BlockHashReader for StateReadTracker<S> {
...
impl<S: HashedPostStateProvider> HashedPostStateProvider for StateReadTracker<S> {
