    ...
    replay_wal(changelog_path, start_version, -1, &mut |entry: &ChangelogEntry| {
        ...
        if entry.version > cosmos_v {
            ...
            cosmos_store.set_latest_version(entry.version).map_err(|e| {
                MptDbError::Other(format!("failed to set cosmos version {}: {e}", entry.version))
            })?;
            ...
            entry.version > evm_v
        {
            ...
            evm.set_latest_version(entry.version).map_err(|e| {
                MptDbError::Other(format!("failed to set EVM version {}: {e}", entry.version))
            })?;
            evm_v = entry.version;
        }
...
mod tests {
    use super::*;
    use mptdb_common::{config::StateStoreConfig, evm_keys::NONCE_KEY_PREFIX};
