...
use mptdb_common::{
    config::WalConfig,
    error::{Result, MptDbError},
};
use mptdb_wal::wal::WalImpl;
use std::path::Path;
...
pub fn encode_wal_entry(entry: &WalEntry) -> Result<Vec<u8>> {
    let count: u32 = entry
        ...
        .len()
        .try_into()
        .map_err(|_| MptDbError::Other(format!("receipt count exceeds u32: {}", entry.receipts.len())))?;
    ...
    for r in &entry.receipts {
        let rlen: u32 = r
            .len()
            .try_into()
            .map_err(|_| MptDbError::Other(format!("receipt length exceeds u32: {}", r.len())))?;
...
pub fn decode_wal_entry(data: &[u8]) -> Result<WalEntry> {
    if data.len() < 12 {
        return Err(MptDbError::Other(format!("WAL entry too short: {} bytes", data.len())));
    }
    ...
    let block_number = u64::from_le_bytes(
        data[offset..offset + 8]
            .try_into()
            .map_err(|_| MptDbError::Other("failed to read block_number".into()))?,
    );
    offset += 8;
    ...
    let num_receipts = u32::from_le_bytes(
        data[offset..offset + 4]
            .try_into()
            .map_err(|_| MptDbError::Other("failed to read receipt count".into()))?,
    );
    offset += 4;
    ...
    for i in 0..num_receipts {
        ...
        let rlen = u32::from_le_bytes(
            data[offset..offset + 4]
                .try_into()
                .map_err(|_| MptDbError::Other(format!("failed to read receipt {i} length")))?,
        ) as usize;
        offset += 4;
