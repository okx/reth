...
- Rust 侧 **SnapshotImporter**（`mptdb-sc/src/composite/importer.rs`）逻辑已实现：按 module 路由、evm 只收 leaf（height==0），与 Go 一致。
...
| 文档中的项 | 说明 | Rust 现状 |
|------------|------|-----------|
| **BuildMemIAVLEVMKey / 迭代器适配** (T9.4.0e) | 迭代器 Key() 返回 memiavl 格式 key | 已实现：`mptdb_common::evm_keys::build_memiavl_evm_key`，FlatKvDbIterator 的 `convert_to_memiavl` + `cached_memiavl_key` 用于导出时统一 key 格式 |
| **FlatKV importer 底层** | 将 state sync 的 leaf 写入 FlatKV | 已实现：`mptdb-sc/src/flatkv/importer.rs` 的 `KvImporter`（add_node、flush、close、commit_global_metadata）与 Go 行为一致 |
| **异步 ChangeSet 真并发** (T9.4b.1) | 文档称“后来已实现” | 以当前代码为准即可；若已实现，则无缺口 |
| **Pruning** | SS 定期 prune 旧版本 | 已实现：`mptdb-ss/src/pruning.rs` 的 `PruningManager` 与 Go 的 prune loop 一致。测试里的 `unimplemented!()` 仅在 **MockStateStore** 的 iterator/reverse_iterator，不影响生产 |
...
| **T9.4.0c/0d** PebbleMetrics / enableMetrics | DB 指标采集 | 生产监控有用；Rust 有 `mptdb_common::metrics::db_metrics`（feature "metrics"），是否全链路接入需单独看 |
...
2. **MemiavlCommitStore 的 Importer**  
   实现 `MemiavlCommitStore::importer(version)`，返回一个实现 `mptdb_traits::sc::Importer` 的类型，内部接收 `ScSnapshotNode` 流（AddModule/AddNode/Close），写入当前 memiavl 目录/DB，与 Go 的 `MultiTreeImporter` + `TreeImporter` 行为一致。
...
| 功能 | 文件/位置 |
|------|-----------|
| Composite create_importer 返回 Err | `mptdb-sc/src/composite/store.rs` 约 238–241 行 |
| SnapshotImporter 实现 | `mptdb-sc/src/composite/importer.rs` |
| MemiavlCommitStore importer 返回 Err | `mptdb-sc/src/memiavl/trait_impl.rs` 约 162–164 行 |
| FlatKV KvImporter | `mptdb-sc/src/flatkv/importer.rs` |
| BuildMemIAVLEVMKey 等价 | `mptdb-common/src/evm_keys.rs`（`build_memiavl_evm_key`），FlatKvDbIterator 使用处：`mptdb-sc/src/flatkv/iterator.rs` |
| PruningManager | `mptdb-ss/src/pruning.rs` |
