...
- **Commands**:
  ```bash
  # Golden tests
  cargo test -p mptdb-sc --test mpt_reth_golden

  # Scale tests (manual)
  cargo test -p mptdb-sc --test mpt_scale -- --ignored --nocapture

  # Micro bench (mpt-db MPT only)
  cargo bench -p mptdb-sc --bench mpt_commit_bench

  # Micro bench with asm-keccak
  cargo bench -p mptdb-sc --bench mpt_commit_bench --features asm-keccak

  # End-to-end: mpt-db MPT vs reth MPT+MDBX
  cargo bench -p xlayer-salt --bench mptdb_vs_reth

  # End-to-end with asm-keccak
  cargo bench -p xlayer-salt --bench mptdb_vs_reth --features asm-keccak
