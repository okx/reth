//! Benchmark: sei-db MPT vs reth MPT+MDBX.
//!
...
/// Run sei-db MPT lane: apply pre-pop (excluded from timing), then process block_bundles.
fn run_seidb_lane(
    pre_pop: &revm_database::BundleState,
    block_bundles: &[revm_database::BundleState],
) -> Duration {
...
fn bench_b4_1_fresh_state(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("seidb_mpt", "100accts_10slots"), |b| {
        b.iter_custom(|iters| {
            let mut total = Duration::ZERO;
            for _ in 0..iters {
                total += run_seidb_lane(&empty_pre_pop, &[bundle.clone()]);
            }
            total
        })
    });
...
fn bench_b4_2_prepop_single_block(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("seidb_mpt", "1K_pre_200_upd"), |b| {
        b.iter_custom(|iters| {
            let mut total = Duration::ZERO;
            for _ in 0..iters {
                total += run_seidb_lane(&pre_pop, &[block.clone()]);
            }
            total
        })
    });
...
fn bench_b4_3_incremental_blocks(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("seidb_mpt", "1K_pre_10x200_upd"), |b| {
        b.iter_custom(|iters| {
            let mut total = Duration::ZERO;
            for _ in 0..iters {
                total += run_seidb_lane(&pre_pop, &blocks);
            }
            total
        })
    });
...
fn bench_b4_4_large_scale(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("seidb_mpt", "200K_pre_10x2K_upd"), |b| {
        b.iter_custom(|iters| {
            let mut total = Duration::ZERO;
            for _ in 0..iters {
                total += run_seidb_lane(&pre_pop, &blocks);
