...
use seidb_common::config::WalConfig;
use seidb_proto::{ChangeSet, ChangelogEntry, KvPair, NamedChangeSet};
use seidb_traits::wal::Wal;
use seidb_wal::{changelog::new_changelog_wal, wal::WalImpl};
...
fn test_prune_lifecycle() {
    ...
    let mut wal: WalImpl<String> = WalImpl::new(
        ...
        |data: &[u8]| {
            Ok(String::from_utf8(data.to_vec()).map_err(|e| SeiDbError::Other(e.to_string()))?)
        },
        config,
        &wal_dir,
    )
...
use seidb_common::error::SeiDbError;
