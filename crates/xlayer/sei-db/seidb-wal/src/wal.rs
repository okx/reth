...
pub struct WalImpl<T: Send + 'static> {
    ...
    async_error_detail: Arc<Mutex<Option<SeiDbError>>>,
...
impl<T: Send + 'static> WalImpl<T> {
    ...
    pub fn new(
        ...
        let async_error_detail: Arc<Mutex<Option<SeiDbError>>> = Arc::new(Mutex::new(None));
        ...
        let worker_handle = std::thread::Builder::new()
            ...
            .map_err(|e| SeiDbError::Other(format!("failed to spawn wal writer thread: {e}")))?;
    ...
    fn check_error(&self) -> Result<()> {
        if self.async_error.load(Ordering::Relaxed) {
            ...
            if let Some(ref e) = *detail {
                Err(SeiDbError::Other(format!("wal async error: {e}")))
            } else {
                Err(SeiDbError::Other("wal async error (unknown)".into()))
            }
        } else {
            Ok(())
        }
    ...
    fn send_truncate(&self, before: bool, index: u64) -> Result<()> {
        ...
        self.truncate_tx
            .send(req)
            .map_err(|_| SeiDbError::Other("wal truncate channel closed".into()))?;
        err_rx
            .recv()
            .map_err(|_| SeiDbError::Other("wal truncate response channel closed".into()))?
    }
}
...
impl<T: Send + 'static> Wal<T> for WalImpl<T> {
    fn write(&self, entry: T) -> Result<()> {
        ...
        if self.async_writes {
            ...
            self.write_tx
                .send(req)
                .map_err(|_| SeiDbError::Other("wal write channel closed".into()))?;
            Ok(())
        } else {
            ...
            err_rx
                .recv()
                .map_err(|_| SeiDbError::Other("wal write response channel closed".into()))?
        }
    }
    ...
    fn close(&mut self) -> Result<()> {
        // If already closed, return cached result.
        {
            ...
            if let Some(ref r) = *cached {
                return match r {
                    Ok(()) => Ok(()),
                    Err(e) => Err(SeiDbError::Other(format!("{e}"))),
                };
            }
        }
        ...
        match &result {
            Ok(()) => *cached = Some(Ok(())),
            Err(e) => *cached = Some(Err(SeiDbError::Other(format!("{e}")))),
        }
        result
    }
}
            ...
            Err(e) => {
                ...
                for (j, r) in requests.into_iter().enumerate() {
                    if let Some(tx) = r.err_tx {
                        if i == j {
                            let _ = tx.send(Err(SeiDbError::Other(format!("{e}"))));
                        } else {
                            let _ = tx.send(Err(SeiDbError::Other(
...
fn handle_truncate(
    ...
    match &result {
        ...
        Err(e) => {
            ...
            if !matches!(e, SeiDbError::NotFound(_)) {
                report_fatal_error(async_error, async_error_detail, e);
            }
        }
    }
...
fn report_fatal_error(
    ...
    *detail = Some(SeiDbError::Other(format!("{err}")));
...
mod tests {
    ...
    fn make_unmarshal() -> impl Fn(&[u8]) -> Result<String> + Send + Sync + 'static {
        |data: &[u8]| {
            String::from_utf8(data.to_vec())
                .map_err(|e| SeiDbError::Other(format!("utf8 error: {e}")))
        }
    }
    ...
    fn test_replay_with_error() {
        ...
        let result = wal.replay(1, 3, &mut |_idx, _entry| {
            count += 1;
            if count == 2 {
                Err(SeiDbError::Other("stop here".into()))
            } else {
                Ok(())
            }
        });
    ...
    fn test_batch_write_marshal_failure() {
        ...
        let marshal = |s: &String| -> Result<Vec<u8>> {
            if s == "FAIL" {
                Err(SeiDbError::Other("marshal failed".into()))
            } else {
