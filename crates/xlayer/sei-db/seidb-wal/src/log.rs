...
impl WalLog {
    ...
    pub fn read(&mut self, index: u64) -> Result<Vec<u8>> {
        ...
        if self.first_index == 0 || index < self.first_index || index > self.last_index {
            return Err(SeiDbError::NotFound(format!("wal entry index {index}")));
        }

        let seg_idx = self
            .find_segment_for_index(index)
            .ok_or_else(|| SeiDbError::NotFound(format!("segment for index {index}")))?;
...
impl WalLog {
    fn validate_write_index(&self, index: u64) -> Result<()> {
        if index == 0 {
            return Err(SeiDbError::Other("wal index must start from 1".into()));
        }
