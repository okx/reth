...
impl MdbxEngine {
    ...
    pub fn open_with_max_size(data_dir: &Path, max_size: usize) -> Result<Self> {
        std::fs::create_dir_all(data_dir)
            .map_err(|e| SeiDbError::Other(format!("create dir {}: {e}", data_dir.display())))?;
        ...
        let env = Environment::builder()
            ...
            .write_map()
            .open(data_dir)
            .map_err(|e| SeiDbError::Other(format!("open mdbx: {e}")))?;

        // Ensure the default (unnamed) database exists.
        {
            let txn =
                env.begin_rw_txn().map_err(|e| SeiDbError::Other(format!("init txn: {e}")))?;
            txn.create_db(None, DatabaseFlags::default())
                .map_err(|e| SeiDbError::Other(format!("create default db: {e}")))?;
            txn.commit().map_err(|e| SeiDbError::Other(format!("init commit: {e}")))?;
        }
...
impl KvEngine for MdbxEngine {
    fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
        let txn = self.env.begin_ro_txn().map_err(|e| SeiDbError::Other(format!("ro txn: {e}")))?;
        let db = txn.open_db(None).map_err(|e| SeiDbError::Other(format!("open db: {e}")))?;
        let result: Option<Vec<u8>> =
            txn.get(db.dbi(), key).map_err(|e| SeiDbError::Other(format!("get: {e}")))?;
        txn.commit().map_err(|e| SeiDbError::Other(format!("ro commit: {e}")))?;
        Ok(result)
    }

    fn set(&self, key: &[u8], value: &[u8], _opts: &WriteOptions) -> Result<()> {
        let txn = self.env.begin_rw_txn().map_err(|e| SeiDbError::Other(format!("rw txn: {e}")))?;
        let db = txn.open_db(None).map_err(|e| SeiDbError::Other(format!("open db: {e}")))?;
        txn.put(db.dbi(), key, value, WriteFlags::empty())
            .map_err(|e| SeiDbError::Other(format!("put: {e}")))?;
        txn.commit().map_err(|e| SeiDbError::Other(format!("commit: {e}")))?;
        Ok(())
    }
    ...
    fn delete(&self, key: &[u8], _opts: &WriteOptions) -> Result<()> {
        ...
        let _ = txn.del(db.dbi(), key, None).map_err(|e| SeiDbError::Other(format!("del: {e}")))?;
        txn.commit().map_err(|e| SeiDbError::Other(format!("commit: {e}")))?;
        Ok(())
    }
    ...
    fn flush(&self) -> Result<()> {
        ...
        self.env.sync(true).map_err(|e| SeiDbError::Other(format!("sync: {e}")))?;
        Ok(())
    }
