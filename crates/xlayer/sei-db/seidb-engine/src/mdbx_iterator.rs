...
use seidb_traits::{kv::KvIterator, types::IterOptions};
use std::borrow::Cow;
...
impl MdbxIterator {
    ...
    pub fn new(env: &Environment, opts: &IterOptions) -> Result<Box<dyn KvIterator>> {
        let txn = env.begin_ro_txn().map_err(|e| SeiDbError::Other(format!("iter ro txn: {e}")))?;
        let db = txn.open_db(None).map_err(|e| SeiDbError::Other(format!("iter open db: {e}")))?;
        let dbi = db.dbi();
    ...
    fn ensure_cursor(&mut self) -> bool {
        ...
        match self.txn.cursor(self.dbi) {
            ...
            Err(e) => {
                self.last_error = Some(SeiDbError::Other(format!("create cursor: {e}")));
                false
            }
        }
    }
