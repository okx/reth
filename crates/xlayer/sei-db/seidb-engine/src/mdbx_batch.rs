...
impl Batch for MdbxBatch {
    ...
    fn commit(&mut self, _opts: &WriteOptions) -> Result<()> {
        ...
        let txn =
            self.env.begin_rw_txn().map_err(|e| SeiDbError::Other(format!("batch rw txn: {e}")))?;
        let db = txn.open_db(None).map_err(|e| SeiDbError::Other(format!("batch open db: {e}")))?;

        for op in &self.ops {
            match op {
                BatchOp::Set { key, value } => {
                    txn.put(db.dbi(), key, value, WriteFlags::empty())
                        .map_err(|e| SeiDbError::Other(format!("batch put: {e}")))?;
                }
                BatchOp::Delete { key } => {
                    let _ = txn
                        .del(db.dbi(), key, None)
                        .map_err(|e| SeiDbError::Other(format!("batch del: {e}")))?;
                }
            }
        }

        txn.commit().map_err(|e| SeiDbError::Other(format!("batch commit: {e}")))?;
