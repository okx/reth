...
/// Uses crash-safe write (tmp + fsync + rename + fsync dir).
...
impl VersionManifest {
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        let data = fs::read(path).map_err(|e| SeiDbError::Other(format!("read manifest: {e}")))?;
        let manifest: Self = serde_json::from_slice(&data)
            .map_err(|e| SeiDbError::Other(format!("parse manifest: {e}")))?;
        Ok(manifest)
    }

    /// Crash-safe save: write tmp, fsync, rename, fsync parent dir.
    pub fn save(&self, path: &Path) -> Result<()> {
        let tmp_path = path.with_extension("tmp");
        let data = serde_json::to_vec_pretty(self)
            .map_err(|e| SeiDbError::Other(format!("serialize manifest: {e}")))?;

        // Step 1: write tmp file
        let mut file = fs::File::create(&tmp_path)
            .map_err(|e| SeiDbError::Other(format!("create manifest.tmp: {e}")))?;
        file.write_all(&data).map_err(|e| SeiDbError::Other(format!("write manifest.tmp: {e}")))?;

        // Step 2: fsync tmp file
        file.sync_all().map_err(|e| SeiDbError::Other(format!("fsync manifest.tmp: {e}")))?;
        drop(file);

        // Step 3: rename tmp -> manifest.json
        fs::rename(&tmp_path, path)
            .map_err(|e| SeiDbError::Other(format!("rename manifest: {e}")))?;
