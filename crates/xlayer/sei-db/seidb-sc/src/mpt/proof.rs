...
fn collect_proof_path(
    ...
    loop {
        let rlp = store
            ...
            .ok_or_else(|| SeiDbError::Other(format!("proof: node not found: {current_hash}")))?;
        proof.push(Bytes::from(rlp.clone()));

        let node =
            decode_node(&rlp).map_err(|e| SeiDbError::Other(format!("proof: decode node: {e}")))?;
...
fn extract_account_from_proof(
    ...
    let node = decode_node(last)
        .map_err(|e| SeiDbError::Other(format!("proof: decode last node: {e}")))?;
...
fn decode_account_leaf(leaf_value: &[u8]) -> Result<alloy_trie::TrieAccount> {
    alloy_trie::TrieAccount::decode(&mut &leaf_value[..])
        .map_err(|e| SeiDbError::Other(format!("decode account leaf RLP: {e}")))
}
...
fn extract_storage_value_from_proof(proof: &[Bytes], slot_path: &Nibbles) -> Result<U256> {
    ...
    let last = &proof[proof.len() - 1];
    let node = decode_node(last)
        .map_err(|e| SeiDbError::Other(format!("proof: decode storage node: {e}")))?;
        ...
        MptNode::Leaf(leaf) => {
            if slot_path.ends_with(&leaf.nibbles) {
                ...
                let value = U256::decode(&mut &leaf.value[..])
                    .map_err(|e| SeiDbError::Other(format!("decode storage value RLP: {e}")))?;
                Ok(value)
            } else {
                Ok(U256::ZERO)
            }
