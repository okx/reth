...
impl<T: Compact> Compact for AlloyEthereumReceipt<T> {
        ...
        B: bytes::BufMut + AsMut<[u8]>,
    {
        ...
        buf.put_slice(&flags);
        if zstd {
            let mut compressor = reth_zstd_compressors::create_receipt_compressor();
    ...
    fn from_compact(buf: &[u8], _len: usize) -> (Self, &[u8]) {
        ...
        if flags.__zstd() != 0 {
            let mut decompressor = reth_zstd_compressors::create_receipt_decompressor();
            let original_buf = buf;
            let decompressed = decompressor.decompress(buf);
            let mut inner_buf: &[u8] = decompressed;
            let (tx_type, new_buf) = T::from_compact(inner_buf, flags.tx_type_len() as usize);
            inner_buf = new_buf;
            let (success, new_buf) = bool::from_compact(inner_buf, flags.success_len() as usize);
            inner_buf = new_buf;
            let (cumulative_gas_used, new_buf) =
                u64::from_compact(inner_buf, flags.cumulative_gas_used_len() as usize);
            inner_buf = new_buf;
            let (logs, _) = Vec::<Log>::from_compact(inner_buf, inner_buf.len());
