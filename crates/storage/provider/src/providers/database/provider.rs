...
use alloy_primitives::{
    ...
    Address, BlockHash, BlockNumber, TxHash, TxNumber, B256, U256,StorageKey,StorageValue
};
...
use reth_storage_api::{
    BlockBodyIndicesProvider, BlockBodyReader, NodePrimitivesProvider, PlainPostState, StateProvider,
            ...
            for (address, storage) in &merged_plain_state.storages {
                ...
                for (storage_key, storage_value) in storage {
                    ...
                    if storage_value.is_zero() {
                        // Delete storage slot
                        tx.set_storage_slot(storage_path, None)
                            .map_err(|e| ProviderError::TrieWitnessError(format!("Failed to set triedb storage slot: {e:?}")))?;
                    } else {
                        // Set storage slot
            ...
            for (address, account_opt) in &merged_plain_state.accounts {
                ...
                if let Some(account) = account_opt {
                    ...
                    tx.set_account(address_path, Some(trie_account))
                        .map_err(|e| ProviderError::TrieWitnessError(format!("Failed to set triedb account: {e:?}")))?;
                } else {
                    // Account is being destroyed
                    tx.set_account(address_path, None)
                        .map_err(|e| ProviderError::TrieWitnessError(format!("Failed to delete triedb account: {e:?}")))?;
                }
            }
